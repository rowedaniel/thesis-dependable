...
suitable for the layer below, in fact implementing them as pure functions
is impractically slow when run on hardware.
As such, each component should ultimately be described as a change in state,
that is a function that takes one argument: the current state, and outputs
the resulting state.

% TODO: should this be a figure?
\begin{lstlisting}[language=isabelle]
definition 's det_program :: "'s $\Rightarrow$ 's"
definition 's ndet_program :: "'s $\Rightarrow$ 's set"
\end{lstlisting}
...
Lastly, the goal of this paper is to provide proofs and definitions for the UDP
layer.
